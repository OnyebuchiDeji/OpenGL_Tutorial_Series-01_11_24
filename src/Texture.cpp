	...
	: m_FilePath(path), m_LocalBuffer(nullptr), m_Width(0), m_Height(0), m_BPP(0)
{
	...
	*	p4: width, p5: height
	*	p6: border color: 0 for black
	*	p7:	format of the image data being supplied 
	*	p8:	Type of data, which is 8 bits, unsigned
	*	p9:	pointer to pixel buffer -- now for this, one can specify a nullptr if not ready to supply data, which allocates
